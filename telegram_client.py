    ...
    async def sync_topics_and_messages(self, db_path: str, media_dir: str, verbose: bool = False, topic_ids: list = None, sync_direction: str = 'forward'):
        ...
        for topic in topics:
            ...
            if sync_direction == 'forward':
                ...
                # но останавливаемся, когда достигаем last_msg_id.
                ...
                while not reached_known: # <-- Внешний цикл с проверкой reached_known
                    ...
                    for msg in messages:
                        ...
                        if msg.id <= last_msg_id:
                            if verbose:
                                print(f"      [REACHED_KNOWN:{topic_id}] Достигли уже известного msg.id={msg.id}, выходим из цикла по сообщениям в пачке")
                    ...
                    # При reverse=True: offset_id должен быть минимальным ID в текущей пачке
                    offset_id = min(msg.id for msg in messages)  # <-- min
                    if verbose:
                ...
                offset_id = 0 # или last_msg_id, если нужно начать с определённого места
                print(f"[*] Обработка темы: [{topic_id}] {topic['title']} (offset: {offset_id}, last_msg_id: {last_msg_id})")
                while True:
                    # Получаем сообщения *в обратном хронологическом порядке*, начиная с offset_id
                    messages = await self.get_messages(topic_id=topic_id, offset_id=offset_id, limit=100, reverse=True)
